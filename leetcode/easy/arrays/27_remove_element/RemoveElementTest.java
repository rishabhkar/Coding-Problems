...
public class RemoveElementTest {
  ...
  public static void main(String[] args) {
    ...
    runCase("official style example one", solver, new int[]{3, 2, 2, 3}, 3, 2, new int[]{2, 2});
        // Order of kept elements is not guaranteed here; compare as multisets instead
        runCaseUnordered("official style example two", solver, new int[]{0, 1, 2, 2, 3, 0, 4, 2}, 2, 5, new int[]{0, 1, 3, 0, 4});
  ...
  private static void runCase(String name, RemoveElement solver, int[] input, int value, int expectedLength, int[] expectedPrefix) {
    ...
    // Variant of runCase that treats the kept prefix as an unordered collection
    private static void runCaseUnordered(String name, RemoveElement solver, int[] input, int value, int expectedLength, int[] expectedPrefix) {
        int[] working = Arrays.copyOf(input, input.length);
        int actualLength = solver.removeElement(working, value);
        assertEquals(name + " length", expectedLength, actualLength);
        assertUnorderedPrefixEquals(name + " prefix (unordered)", expectedPrefix, working, actualLength);
        testsRun++;
    }
  ...
  private static void assertPrefixEquals(String name, int[] expectedPrefix, int[] actual, int length) {
    ...
    private static void assertUnorderedPrefixEquals(String name, int[] expectedPrefix, int[] actual, int length) {
        int[] actualPrefix = Arrays.copyOf(actual, length);
        int[] expectedCopy = Arrays.copyOf(expectedPrefix, expectedPrefix.length);
        Arrays.sort(expectedCopy);
        Arrays.sort(actualPrefix);
        if (!Arrays.equals(expectedCopy, actualPrefix)) {
            throw new AssertionError(name + " expectedUnorderedPrefix=" + Arrays.toString(expectedCopy) + " actualUnorderedPrefix=" + Arrays.toString(actualPrefix));
        }
    }
